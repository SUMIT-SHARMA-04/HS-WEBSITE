...
const heroStats = [
  ...
  { icon: Users, num: '2K+', label: 'Guests Served' }, // FIX: Updated guests served metric
  ...
  return (
    ...
    >
      <div
        ...
        style={{ backgroundImage: 'url(https://images.pexels.com/photos/10135116/pexels-photo-10135116.jpeg?auto=compress&cs=tinysrgb&h=650&w=940)' }}
      />
      ...
      <div className="relative z-10 text-center px-6 max-w-4xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-4 mb-14 animate-slide-up" style={{ animationDelay: '0.3s' }}>
          <a href="#book" className="btn-gold font-medium px-8 py-3.5 rounded-full shadow-lg">Book a Table</a>
          <a href="#menu" className="btn-gold-outline font-medium px-8 py-3.5 rounded-full">View Menu</a>
        </div>
        ...
        <div className="flex items-center justify-center gap-8 md:gap-14 animate-slide-up" style={{ animationDelay: '0.45s' }}>
          {heroStats.map(({ icon: Icon, num, label }) => (
            <div key={label} className="text-center">
              ...
              <p className="font-serif text-2xl md:text-3xl font-bold text-cream-100">{num}</p>
              <p className="text-cream-300/70 text-xs md:text-sm mt-1 uppercase tracking-wide">{label}</p>
            </div>
          ))}
        </div>
      </div>

      <a href="#about" className="absolute bottom-8 left-1/2 -translate-x-1/2 text-gold-400/60 hover:text-gold-300 transition-colors animate-bounce" aria-label="Scroll down">
